...
#define APP_VERSION     60064U
	...
	Reg[13]
	Reg[14]
	Reg[15]	APP程序版本号，十进制60062
	Reg[16]	保留
	
	06只写
	reg[17]	0x11写入0x1234进入升级模式
	...
	Reg[23]	0x17地址		0-99
	Reg[24]	0x18波特率		0=9600bps, 1=19200bps,2=115200bps
...
// Modbus 3鍙峰姛鑳界爜鍑芥暟
static void Modbus_Send_Exception(u8 func_code, u8 exception_code);
...
//	u16 response_size;
	...
	/* Modbus 03 request must stay inside Reg[128] and sendbuf[100]. */
	if (modbus.recount != 8 || Reglen == 0 || Regadd >= 128 ||
		Reglen > (128 - Regadd))
	{
...
static void Modbus_Send_Exception(u8 func_code, u8 exception_code)
{
	...
	modbus.sendbuf[1] = func_code | 0x80;
	modbus.sendbuf[2] = exception_code;
